...
A.colorFromRgb=function(G,J){var E=G.toLowerCase().match(/^(rgba?|hsla?)\(([\s\.\-,%0-9]+)\)/);
if(E){var M=E[2].split(/\s*,\s*/),F=M.length,R=E[1];
if((R=="rgb"&&F==3)||(R=="rgba"&&F==4)){var D=M[0];
if(D.charAt(D.length-1)=="%"){var N=A.map(M,function(H){return parseFloat(H)*2.56
});
if(F==4){N[3]=M[3]
}return A.colorFromArray(N,J)
}return A.colorFromArray(M,J)
}if((R=="hsl"&&F==3)||(R=="hsla"&&F==4)){var O=((parseFloat(M[0])%360)+360)%360/360,I=parseFloat(M[1])/100,K=parseFloat(M[2])/100,P=K<=0.5?K*(I+1):K+I-K*I,Q=2*K-P,N=[B(Q,P,O+1/3)*256,B(Q,P,O)*256,B(Q,P,O-1/3)*256,1];
if(F==4){N[3]=M[3]
}return A.colorFromArray(N,J)
}}return null
};
var C=function(F,D,E){F=Number(F);
return isNaN(F)?E:F<D?D:F>E?E:F
};
A.Color.prototype.sanitize=function(){var D=this;
D.r=Math.round(C(D.r,0,255));
D.g=Math.round(C(D.g,0,255));
D.b=Math.round(C(D.b,0,255));
D.a=C(D.a,0,1);
return this
}
})();
A.colors.makeGrey=function(C,B){return A.colorFromArray([C,C,C,B])
};
