...
if(aipo.userAgent.isAndroid()){dojo.query("input,select,button").forEach(function(D,C){D.disabled=true
})
}},_closeDropDown:function(){aimluck.widget.Dropdown.prototype._closeDropDown.apply(this);
if(aipo.userAgent.isAndroid()){dojo.query("input,select,button:not(.disabled)").forEach(function(D,C){D.disabled=false
})
...
if(this.initValue!=""){var L=this.initValue.split("/");
if(L.length==3){var J=L[0];
var G=L[1]-1;
var K=L[2];
var I=dojo.byId(this.dateId);
I.value=this.initValue;
this.dropDown.clearDate();
this.dropDown.setValue(new Date(J,G,K))
}}else{this.dropDown.disabledCalendar(true)
}},onCheckBlank:function(B){this.dropDown.disabledCalendar(dojo.byId(this.dateId+"_flag").checked)
}})
};