...
aipo.tutorial.showDialog=function(E,G,F){var H=dijit.byId("imageDialog");
dojo.query(".roundBlockContent").addClass("mb_dialoghide");
dojo.query("#imageDialog").addClass("mb_dialog");
if(!H){H=new aipo.widget.TutorialDialog({widgetId:"imageDialog",_portlet_id:G,_callback:F},"imageDialog")
}else{H.setCallback(G,F)
}if(H){H.setHref(E);
H.show()
}};
aipo.tutorial.hideDialog=function(){var B=dijit.byId("imageDialog");
if(B){B.hide()
}};
aipo.tutorial.onLoadImage=function(D){var C=dojo.byId("imageDialog");
C.style.visibility="hidden";
C.style.width=1050+"px";
C.style.height=650+"px";
dijit.byId("imageDialog")._position();
C.style.visibility="visible"
};
aipo.tutorial.nextPage=function(){var C=dojo.byId("page_tutorial");
var D=C.value-0;
dojo.byId("popupImage"+D).style.display="none";
if(D==1){dojo.byId("tutorial_prev").style.display=""
}D++;
dojo.byId("popupImage"+D).style.display="";
if(D==3){dojo.byId("tutorial_next").style.display="none"
}C.value=D+""
};
aipo.tutorial.prevPage=function(){var C=dojo.byId("page_tutorial");
var D=C.value-0;
dojo.byId("popupImage"+D).style.display="none";
if(D==3){dojo.byId("tutorial_next").style.display=""
}D--;
dojo.byId("popupImage"+D).style.display="";
if(D==1){dojo.byId("tutorial_prev").style.display="none"
}C.value=D+""
};
