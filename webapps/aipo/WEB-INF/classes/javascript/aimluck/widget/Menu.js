...
},makeMenu:function(F){var E=this;
var D=0;
dojo.forEach(F,function(C){if(C.submenu){var A=new aimluck.widget.Menu({id:C.caption,style:"display: none;"});
dojo.forEach(C.submenu,function(G){if(G!=null){if(G.caption){A.addChild(new aimluck.widget.Menuitem({label:G.caption,url:G.url,iconClass:G.iconClass}))
}else{A.addChild(new aimluck.widget.Menuseparator())
}}});
var H="";
if(E.selectedIndex==parseInt(D)){H+="menuBarItemSelected"
}var B=new aimluck.widget.ComboButton({label:C.caption,iconClass:C.iconClass,dropDown:A,url:C.url,itemClass:H});
B.addChild(A);
E.addChild(B)
}else{if(C.url){var H="";
if(E.selectedIndex==D){H+="menuBarItemSelected"
}var B=new aimluck.widget.MenuButton({id:C.caption+"_Button"+D,label:C.caption,url:C.url,iconClass:C.iconClass,itemClass:H});
E.addChild(B)
}else{E.addChild(new aimluck.widget.ToolbarSeparator())
}}D++
})
}})
};