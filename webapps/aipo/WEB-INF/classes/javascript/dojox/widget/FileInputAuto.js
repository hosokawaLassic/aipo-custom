...
}},setMessage:function(B){if(!dojo.isIE){this.overlay.innerHTML=B
}},_sendFile:function(E){if(!this.fileInput.value||this._sent){return 
...
var F=dojo.clone(this.fileInput);
D.appendChild(this.fileInput);
dojo.body().appendChild(D);
dojo.io.iframe.send({url:this.url+"?name="+this.name,form:D,handleAs:"text",handle:dojo.hitch(this,"_handleSend")})
},_handleSend:function(D,C){if(!dojo.isIE){this.overlay.innerHTML=""
}this._sent=true;
dojo.style(this.overlay,"opacity","0");
