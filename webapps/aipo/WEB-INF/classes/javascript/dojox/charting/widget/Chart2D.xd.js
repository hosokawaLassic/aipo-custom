dojo._xdResourceLoaded({depends:[["provide","dojox.charting.widget.Chart2D"],["require","dijit._Widget"],["require","dojox.charting.Chart2D"],["require","dojox.lang.functional"]],defineResource:function(dojo){if(!dojo._hasResource["dojox.charting.widget.Chart2D"]){dojo._hasResource["dojox.charting.widget.Chart2D"]=true;
dojo.provide("dojox.charting.widget.Chart2D");
dojo.require("dijit._Widget");
dojo.require("dojox.charting.Chart2D");
dojo.require("dojox.lang.functional");
(function(){var collectAxisParams,collectPlotParams,collectDataParams,notNull=function(o){return o
},df=dojox.lang.functional,du=dojox.lang.utils;
dojo.declare("dojox.charting.widget.Chart2D",dijit._Widget,{theme:null,margins:null,stroke:null,fill:null,buildRendering:function(){var n=this.domNode=this.srcNodeRef;
...
if(this.theme){c.setTheme(this.theme)
}dojo.forEach(axes,function(axis){c.addAxis(axis.name,axis.kwArgs)
});
dojo.forEach(plots,function(plot){c.addPlot(plot.name,plot.kwArgs)
});
var render=df.foldl(series,function(render,series){if(series.type=="data"){c.addSeries(series.name,series.data,series.kwArgs);
render=true
}else{c.addSeries(series.name,[0],series.kwArgs);
var kw={};
du.updateWithPattern(kw,series.kwArgs,{query:"",queryOptions:null,start:0,count:1,sort:[]},true);
dojo.mixin(kw,{onComplete:function(data){var values;
if("valueFn" in series.kwArgs){var fn=series.kwArgs.valueFn;
values=dojo.map(data,function(x){return fn(series.data.getValue(x,series.field,0))
})
}else{values=dojo.map(data,function(x){return series.data.getValue(x,series.field,0)
})
...
if(render){c.render()
}},resize:function(box){dojo.marginBox(this.domNode,box);
this.chart.resize()
}});
...
if(type){if(dojox.charting.axis2d[type]){type="dojox.charting.axis2d."+type
}var axis=eval("("+type+")");
if(axis){kw.type=axis
}}else{type="dojox.charting.axis2d.Default"
}var dp=eval("("+type+".prototype.defaultParams)");
for(var x in dp){if(x in kw){continue
}var attr=node.getAttribute(x);
kw[x]=du.coerceType(dp[x],attr==null?dp[x]:attr)
}var op=eval("("+type+".prototype.optionalParams)");
for(var x in op){if(x in kw){continue
}var attr=node.getAttribute(x);
if(attr!=null){kw[x]=du.coerceType(op[x],attr)
}}return o
};
collectPlotParams=function(node){var name=node.getAttribute("name"),type=node.getAttribute("type");
if(!name){return null
}var o={name:name,kwArgs:{}},kw=o.kwArgs;
if(type){if(dojox.charting.plot2d[type]){type="dojox.charting.plot2d."+type
}var plot=eval("("+type+")");
if(plot){kw.type=plot
}}else{type="dojox.charting.plot2d.Default"
}var dp=eval("("+type+".prototype.defaultParams)");
for(var x in dp){if(x in kw){continue
}var attr=node.getAttribute(x);
kw[x]=du.coerceType(dp[x],attr==null?dp[x]:attr)
...
collectDataParams=function(node){var name=node.getAttribute("name");
if(!name){return null
