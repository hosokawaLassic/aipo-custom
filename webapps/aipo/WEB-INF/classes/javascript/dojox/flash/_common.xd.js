dojo._xdResourceLoaded({depends:[["provide","dojox.flash._common"]],defineResource:function(dojo){if(!dojo._hasResource["dojox.flash._common"]){dojo._hasResource["dojox.flash._common"]=true;
dojo.provide("dojox.flash._common");
dojox.flash=function(){};
dojox.flash={flash6_version:null,flash8_version:null,ready:false,_visible:true,_loadedListeners:new Array(),_installingListeners:new Array(),setSwf:function(fileInfo){if(!fileInfo){return 
}if(fileInfo.flash6){this.flash6_version=fileInfo.flash6
}if(fileInfo.flash8){this.flash8_version=fileInfo.flash8
...
if(dojox.flash._loadedListeners.length>0){for(var i=0;
i<dojox.flash._loadedListeners.length;
i++){dojox.flash._loadedListeners[i].call(null)
}}},installing:function(){if(dojox.flash._installingListeners.length>0){for(var i=0;
i<dojox.flash._installingListeners.length;
i++){dojox.flash._installingListeners[i].call(null)
}}},_initialize:function(){var installer=new dojox.flash.Install();
...
dojox.flash.Info.prototype={version:-1,versionMajor:-1,versionMinor:-1,versionRevision:-1,capable:false,commVersion:6,installing:false,isVersionOrAbove:function(reqMajorVer,reqMinorVer,reqVer){reqVer=parseFloat("."+reqVer);
if(this.versionMajor>=reqMajorVer&&this.versionMinor>=reqMinorVer&&this.versionRevision>=reqVer){return true
}else{return false
}},getResourceList:function(swfloc6,swfloc8){var results=[];
var swfloc=swfloc6;
results.push(swfloc);
swfloc=swfloc+"?baseRelativePath="+escape(dojo.baseUrl);
results.push(swfloc);
swfloc+="'%20'%20quality=";
results.push(swfloc);
swfloc=swfloc8;
results.push(swfloc);
swfloc+="?baseRelativePath="+escape(dojo.baseUrl);
results.push(swfloc);
swfloc+="'%20'%20quality=";
results.push(swfloc);
...
for(var testVersion=25;
testVersion>0;
testVersion--){if(dojo.isIE){versionStr=VBGetSwfVer(testVersion)
}else{versionStr=this._JSFlashInfo(testVersion)
}if(versionStr==-1){this.capable=false;
return 
}else{if(versionStr!=0){var versionArray;
if(dojo.isIE){var tempArray=versionStr.split(" ");
...
this.capable=true;
break
}}}},_JSFlashInfo:function(testVersion){if(navigator.plugins!=null&&navigator.plugins.length>0){if(navigator.plugins["Shockwave Flash 2.0"]||navigator.plugins["Shockwave Flash"]){var swVer2=navigator.plugins["Shockwave Flash 2.0"]?" 2.0":"";
...
dojox.flash.Embed=function(visible){this._visible=visible
};
...
default:return"http";
break
}},write:function(flashVer,doExpressInstall){doExpressInstall=!!doExpressInstall;
var containerStyle="";
containerStyle+=("width: "+this.width+"px; ");
containerStyle+=("height: "+this.height+"px; ");
if(this._visible==false){containerStyle+="position: absolute; z-index: 10000; top: -1000px; left: -1000px; "
}var objectHTML;
var swfloc;
if(flashVer==6){swfloc=dojox.flash.flash6_version;
...
}else{swfloc=dojox.flash.flash8_version;
...
dojox.flash.Communicator.prototype={_writeFlash6:function(){var id=dojox.flash.obj.id;
...
document.writeln("</SCRIPT> ")
}},_writeFlash8:function(){},_handleFSCommand:function(command,args){if((command)&&dojo.isString(command)&&(/^FSCommand:(.*)/.test(command)==true)){command=command.match(/^FSCommand:(.*)/)[1]
...
var callFunc=function(){return dojox.flash.comm._call(functionName,arguments)
};
...
},_fscommandReady:function(){var plugin=dojox.flash.obj.get();
plugin.SetVariable("fscommandReady","true")
},_call:function(functionName,args){var plugin=dojox.flash.obj.get();
...
data=eval('"'+data+'"');
return data
},_chunkArgumentData:function(value,argIndex){var plugin=dojox.flash.obj.get();
var numSegments=Math.ceil(value.length/1024);
for(var i=0;
i<numSegments;
i++){var startCut=i*1024;
...
}},_chunkReturnData:function(){var plugin=dojox.flash.obj.get();
var numSegments=plugin.getReturnLength();
var resultsArray=new Array();
for(var i=0;
i<numSegments;
i++){var piece=plugin.CallFunction('<invoke name="chunkReturnData" returntype="javascript"><arguments><number>'+i+"</number></arguments></invoke>");
if(piece=='""'||piece=="''"){piece=""
}else{piece=piece.substring(1,piece.length-1)
}resultsArray.push(piece)
}var results=resultsArray.join("");
return results
},_execFlash:function(methodName,methodArgs){var plugin=dojox.flash.obj.get();
plugin.startExec();
plugin.setNumberArguments(methodArgs.length);
for(var i=0;
i<methodArgs.length;
i++){this._chunkArgumentData(methodArgs[i],i)
...
if(isMac&&(!dojox.flash.info.isVersionOrAbove(8,0,0))){return true
...
if(dojox.flash.info.capable==false){var installObj=new dojox.flash.Embed(false);
installObj.write(8)
}else{if(dojox.flash.info.isVersionOrAbove(6,0,65)){var installObj=new dojox.flash.Embed(false);
...
}}},_onInstallStatus:function(msg){if(msg=="Download.Complete"){dojox.flash._initialize()
}else{if(msg=="Download.Cancelled"){alert("This content requires a more recent version of the Macromedia  Flash Player.");
window.location.href=dojox.flash.Embed.protocol()+"://www.macromedia.com/go/getflashplayer"
}else{if(msg=="Download.Failed"){alert("There was an error downloading the Flash Player update. Please try again later, or visit macromedia.com to download the latest version of the Flash plugin.")
}}}}};
