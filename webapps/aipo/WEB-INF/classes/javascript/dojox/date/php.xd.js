dojo._xdResourceLoaded({depends:[["provide","dojox.date.php"],["require","dojo.date"]],defineResource:function(B){if(!B._hasResource["dojox.date.php"]){B._hasResource["dojox.date.php"]=true;
B.provide("dojox.date.php");
B.require("dojo.date");
dojox.date.php.format=function(H,G,A){var F=new dojox.date.php.DateFormat(H);
return F.format(G,A)
};
dojox.date.php.DateFormat=function(A){this.date=A
};
B.extend(dojox.date.php.DateFormat,{weekdays:["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],weekdays_3:["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],months:["January","February","March","April","May","June","July","August","September","October","November","December"],months_3:["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],monthdays:[31,28,31,30,31,30,31,31,30,31,30,31],format:function(G,A){var H=[];
for(var J=0;
J<G.length;
J++){var I=G.charAt(J);
if(A&&typeof A[I]=="function"){H.push(A[I].call(this))
}else{if(typeof this[I]=="function"){H.push(this[I]())
}else{H.push(I)
}}}return H.join("")
},d:function(){var A=this.j();
return(A.length==1)?"0"+A:A
},D:function(){return this.weekdays_3[this.date.getDay()]
},j:function(){return this.date.getDate()+""
},l:function(){return this.weekdays[this.date.getDay()]
},N:function(){var A=this.w();
return(!A)?7:A
},S:function(){switch(this.date.getDate()){case 11:case 12:case 13:return"th";
case 1:case 21:case 31:return"st";
case 2:case 22:return"nd";
case 3:case 23:return"rd";
default:return"th"
}},w:function(){return this.date.getDay()+""
},z:function(){var A=this.date.getTime()-new Date(this.date.getFullYear(),0,1).getTime();
return Math.floor(A/86400000)+""
},W:function(){var I;
var N=new Date(this.date.getFullYear(),0,1).getDay()+1;
var M=this.date.getDay()+1;
var A=parseInt(this.z());
if(A<=(8-N)&&N>4){var J=new Date(this.date.getFullYear()-1,this.date.getMonth(),this.date.getDate());
if(N==5||(N==6&&B.date.isLeapYear(J))){I=53
}else{I=52
}}else{var K;
if(Boolean(this.L())){K=366
}else{K=365
}if((K-A)<(4-M)){I=1
}else{var L=A+(7-M)+(N-1);
I=Math.ceil(L/7);
if(N>4){--I
}}}return I
},F:function(){return this.months[this.date.getMonth()]
},m:function(){var A=this.n();
return(A.length==1)?"0"+A:A
},M:function(){return months_3[this.date.getMonth()]
},n:function(){return this.date.getMonth()+1+""
},t:function(){return(Boolean(this.L())&&this.date.getMonth()==1)?29:this.monthdays[this.getMonth()]
},L:function(){return(B.date.isLeapYear(this.date))?"1":"0"
},o:function(){},Y:function(){return this.date.getFullYear()+""
},y:function(){return this.date.getFullYear.substsring(2,4)
},a:function(){return this.date.getHours()>=12?"pm":"am"
},b:function(){return this.a().toUpperCase()
},B:function(){var A=this.date.getTimezoneOffset()+60;
var F=(this.date.getHours()*3600)+(this.date.getMinutes()*60)+this.getSeconds()+(A*60);
var E=Math.abs(Math.floor(F/86.4)%1000)+"";
while(E.length<2){E="0"+E
}return E
},g:function(){return(this.date.getHours()>12)?this.date.getHours()-12+"":this.date.getHours()+""
},G:function(){return this.date.getHours()+""
},h:function(){var A=this.g();
return(A.length==1)?"0"+A:A
},H:function(){var A=this.G();
return(A.length==1)?"0"+A:A
},i:function(){var A=this.date.getMinutes()+"";
return(A.length==1)?"0"+A:A
},s:function(){var A=this.date.getSeconds()+"";
return(A.length==1)?"0"+A:A
},e:function(){return B.date.getTimezoneName(this.date)
},I:function(){},O:function(){var A=Math.abs(this.date.getTimezoneOffset());
var F=Math.floor(A/60)+"";
var E=(A%60)+"";
if(F.length==1){F="0"+F
}if(E.length==1){F="0"+E
}return((this.date.getTimezoneOffset()<0)?"+":"-")+F+E
},P:function(){var A=this.O();
return A.substring(0,2)+":"+A.substring(2,4)
},T:function(){return this.e().substring(0,3)
},Z:function(){return this.date.getTimezoneOffset()*-60
},c:function(){return this.Y()+"-"+this.m()+"-"+this.d()+"T"+this.h()+":"+this.i()+":"+this.s()+this.P()
},r:function(){return this.D()+", "+this.d()+" "+this.M()+" "+this.Y()+" "+this.H()+":"+this.i()+":"+this.s()+" "+this.O()
},U:function(){return Math.floor(this.date.getTime()/1000)
}})
}}});